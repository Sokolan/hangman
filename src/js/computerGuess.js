...
const ComputerGuess = (wordLength) => {
  ...
  const getNextLetterGuess = () => {
    if (potencialWords.length === 1) return potencialWords[0][0];
    const hist = {};
    ...
    const maxValue = Object.entries(hist).reduce(
      (accumulated, current) =>
        current[1] > accumulated[1] && !guessedLetters.has(current[0]) ? current : accumulated,
      ["a", 0],
    );

    guessedLetters.add(maxValue[0]);
    return maxValue[0];
  };

  const isWordFound = () =>
    currentWord.indexOf("*") === -1 || potencialWords.length === 1;
